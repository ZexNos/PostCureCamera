...
- `DoorOpen`: при открытой двери **UV немедленно OFF**, нагрев — **пауза**. После закрытия двери требуется подтверждение пользователем, после чего цикл может быть продолжен.  
- `OverTemp`: при перегреве — OFF нагрев/UV, состояние ERROR, включается продувка (в DRY — мягкая, чтобы не убить филамент). Ошибка требует явного сброса.  
- `SensorFault`: при отказе критичных датчиков — останов процесса, OFF нагревателя, мягкая продувка камеры, состояние ERROR. Пресеты недоступны до восстановления валидных датчиков; допускается только ручной режим при исправном основном датчике и управляемом нагревателе.  
- `HeaterNoEffect`: если PWM>20% дольше N секунд, а ∆T < threshold → ошибка нагревателя.  
- `PURGE`: режим продувки без нагрева и UV, используется для охлаждения нагревателя и удаления запаха после CURE/DRY или ERROR, а также по ручному запросу.  

Реакции должны **однозначно отображаться**: экран статуса/ошибки + RGB + MQTT-логи.
